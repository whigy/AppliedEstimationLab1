...
x = [1 0.5]'; % state of the system
u = 0; % control

% the simulated noise 

wStdP = 0.01; % Noise on simulated position (noise for the 1st dimension of x)
wStdV = 0.1;   % Noise on simulated velocity (noise for the 2nd dimension of x)
vStd = 0.1;    % Simulated measurement noise on position (noise for measurement z)

%%%%%%%%%%%%%%%%%%%%%%%%%%
% The Kalman Filter modeled uncertainties and initial values

xhat = [-2 0]'% *10; 
P = eye(2)*100; % Covariance of Matrix of state transition probability P(k+1|k) = A * P * A^T + G * R * G^T 
G = eye(2); %???????????????
D = 1;  %???????????
kR = 1;
kQ = 1;
R = diag([0.01^2 0.1^2]) * kR; 
Q = 0.1^2 * kQ;
plotname = ['./plot/Lab1_3-1_Q3_(',num2str(kR),')R(',num2str(kQ),')Q.png']
n = 100;
...
for k = 1:n
    x = A * x + B * u + [wStdP*randn(1,1); wStdV*randn(1,1)];  %randn Normally distributed pseudorandom numbers.
    y = C * x + D * vStd*randn(1,1);
    X(:,k+1) = x;

    % Prediction
    xhat = A * xhat + B * u; % Unnoised(?) perdiction of state (mean) before incorporating z_t 
    P = A * P * A' + G * R * G'; 
